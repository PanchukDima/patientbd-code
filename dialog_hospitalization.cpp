...
void Dialog_hospitalization::send_data()
{
    ...
    QString date_up = ui->dateEdit_date_up->date().toString("MM.dd.yyyy");
    QString date_down = ui->dateEdit_date_down->date().toString("MM.dd.yyyy");
    if(db.open())
    {
    switch (global_param) {
    case 0:
        ...
        {
            query.exec("INSERT INTO test.hospitalization( medcard_id, staff_add_id, who_wrote, date_up, date_down, delete_row) VALUES ( '"+global_id+"', '"+global_staff+"', '"+who+"', '"+date_up+"', '"+date_down+"', '0');");
        }
        else
        {
            query.exec("INSERT INTO test.hospitalization( medcard_id, staff_add_id, who_wrote, date_up, delete_row) VALUES ( '"+global_id+"', '"+global_staff+"', '"+who+"', '"+date_up+"', '0');");
        }

        query.exec("INSERT INTO test.logs(staff_add_id, date_add, text) VALUES ('"+global_staff+"', '"+QDate::currentDate().toString("MM.dd.yyyy")+"', 'Добавлена госпитализация в медкарту № "+global_id+"')");
    ...
    case 1:
        ...
        query.exec("INSERT INTO test.logs(staff_add_id, date_add, text) VALUES ('"+global_staff+"', '"+QDate::currentDate().toString("MM.dd.yyyy")+"', 'Изменена госпитализация № "+global_id+"')");
        Dialog_hospitalization::accept();
        break;
    }
